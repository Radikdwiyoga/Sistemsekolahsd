  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16 font-sans flex flex-col" id="parent-view">
      ...
      <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 mt-6 space-y-6">
        ...
        <section className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white p-6 sm:p-8 rounded-3xl shadow-md relative overflow-hidden">
                    ...
                    {isPaid && pay.proof_file_name && (
                      <div className="pt-1">
                        ...
                        >
                          ...
                          <span className="max-w-[200px] truncate">Bukti: {pay.proof_file_name}</span>
                        </a>
                      </div>
                    )}
                  </div>
                    ...
                    {pay.status === 'belum_lunas' && (
                      <div className="flex flex-col gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-200/60 max-w-xs">
                        ...
                        <div className="flex items-center gap-1.5">
                          <label className="flex-grow flex items-center justify-between px-2.5 py-1.5 bg-white hover:bg-slate-100 border border-slate-200 rounded-lg text-[10px] font-bold text-slate-600 cursor-pointer transition-colors max-w-[150px]">
                            <span className="truncate">{selectedFile?.name || 'Pilih File...'}</span>
                            <Upload className="w-3.5 h-3.5 text-slate-400 shrink-0 ml-1" />
                            <input 
                              type="file"
                              accept="image/*,.pdf"
                              onChange={(e) => handleProofFileChange(pay.id, e)}
                              className="hidden"
                            />
                          </label>
                          {selectedFile && (
          ...
          <div className="bg-slate-50 border border-slate-150 rounded-2xl overflow-hidden flex flex-col h-80 mt-4">
            ...
            <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-slate-150 flex gap-2">
              <input 
                ...
                className="flex-grow px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
              <button 
                type="submit"
